...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['_tok']) || $_POST['_tok'] !== ($_SESSION['backup_tok'] ?? '')) {
        ...
        if ($action === 'run_backup') {
            // Run the backup script via proc_open so we capture output without
            // PHP exec() potentially being disabled.
            $cmd = PHP_BINARY . ' ' . escapeshellarg($backupScript);
            $desc = [1 => ['pipe','w'], 2 => ['pipe','w']];
            $proc = proc_open($cmd, $desc, $pipes, $ROOT);
            if (!is_resource($proc)) {
                $error = 'Could not launch backup process. Check server permissions.';
            } else {
                ...
                if ($exit === 0) {
                    $message = 'Backup completed. ' . htmlspecialchars(trim($stdout));
                } else {
                    $error = 'Backup failed (exit ' . $exit . '): ' . htmlspecialchars(trim($stderr ?: $stdout));
                }
            }
            ...
            if (!preg_match('#^backups[/\\\\]\d{4}[/\\\\]\d{2}[/\\\\]db-\d{8}-\d{6}\.sql\.gz$#', $relPath)) {
                ...
                if (!is_file($absPath)) {
                    ...
                    // Run the restore script — it requires --confirm flag explicitly
                    $cmd = PHP_BINARY . ' ' . escapeshellarg($restoreScript)
                         . ' --file=' . escapeshellarg($relPath)
                         . ' --confirm';
                    $desc = [1 => ['pipe','w'], 2 => ['pipe','w']];
                    $proc = proc_open($cmd, $desc, $pipes, $ROOT);
                    if (!is_resource($proc)) {
                        $error = 'Could not launch restore process.';
                    } else {
                        $stdout = stream_get_contents($pipes[1]);
                        $stderr = stream_get_contents($pipes[2]);
                        fclose($pipes[1]); fclose($pipes[2]);
                        $exit = proc_close($proc);
                        if ($exit === 0) {
                            $message = 'Restore completed successfully. ' . htmlspecialchars(trim($stdout));
                        } else {
                            $error = 'Restore failed (exit ' . $exit . '): ' . htmlspecialchars(trim($stderr ?: $stdout));
                        }
                    }
                }
            }
...
<div class="content">
    <div class="content-header">
        ...
        <p style="color:#6b7280;margin:4px 0 0;">Run backups, download files, restore from a previous snapshot, or monitor backup freshness.</p>
    </div>
