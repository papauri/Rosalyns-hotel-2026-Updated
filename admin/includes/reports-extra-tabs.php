...
<div class="tab-content <?php echo $active_tab === 'voids' ? 'active' : ''; ?>" id="tab-voids">
    ...
    <div class="rx-section">
        ...
        <?php foreach ($voids['list'] as $v): ?>
            <tr><td><?php echo htmlspecialchars($v['voided_at']); ?></td><td><strong><?php echo htmlspecialchars($v['reference']); ?></strong></td><td><?php echo htmlspecialchars($v['order_type']); ?></td><td><?php echo htmlspecialchars($v['created_by_name'] ?: '—'); ?></td><td><?php echo htmlspecialchars($v['voided_by_name'] ?: '—'); ?></td><td class="num"><?php echo $rp_currency.' '.number_format((float)$v['total_amount'], 2); ?></td><td style="max-width:300px; font-size:12px;"><?php echo htmlspecialchars($v['void_reason']); ?></td><td><a href="order-lifecycle.php?id=<?php echo (int)$v['id']; ?>" style="color:#8B7355; font-size:12px;"><i class="fas fa-stream"></i></a></td></tr>
        <?php endforeach; ?>
        </tbody></table>
        <?php endif; ?>
    </div>
