...
try {
    switch ($action) {
        ...
        case 'get_recipe': {
            ...
            if ($mid <= 0) { throw new InvalidArgumentException('menu_item_id required'); }
        ...
        case 'save_recipe': {
            ...
            $check->execute([$mid]);
            if (!$check->fetchColumn()) { throw new RuntimeException('Menu item not found'); }
        ...
        case 'add_ingredient': {
            ...
            if ($name === '') { throw new InvalidArgumentException('Ingredient name required'); }
            if ($unit === '') { $unit = 'g'; }
