...
try {
    ...
    error_log('get-housekeeping-audit.php init error: ' . $e->getMessage());
...
if (!function_exists('hasPermission') || !isset($user['id'])) {
    error_log('get-housekeeping-audit.php: hasPermission or user not available');
    header('Content-Type: application/json');
...
if ($assignmentId <= 0) {
    header('Content-Type: application/json');
    echo json_encode(['error' => 'Invalid assignment ID', 'logs' => []]);
    exit;
}
...
try {
    ...
    if (!function_exists('getHousekeepingAuditLog')) {
        error_log('get-housekeeping-audit.php: getHousekeepingAuditLog function not found');
        header('Content-Type: application/json');
    ...
    error_log('get-housekeeping-audit.php error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    header('Content-Type: application/json');
