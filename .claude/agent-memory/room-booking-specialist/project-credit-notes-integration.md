...
- Apply credit at booking time: `admin/create-booking.php` — payment step has a "Guest Credit" panel that calls `create-booking.php?ajax=guest_credit_lookup&email=` (served from the page itself so it uses booking-staff auth, NOT the `invoices`-gated credit-notes API). Selected `apply_credit_note_ids[]` are applied post-commit, greedily across the created bookings, capped at each booking's amount_due. Security: only credit notes whose `guest_email` matches the booking email are applied.
